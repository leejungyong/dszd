...
Page({
  ...
  onAccelerometerChange(res) {
    ...
    // let s=wx.getStorageSync('state')
      if (res.x > .7 && res.y > .7) {
        ...
        that.startAnimation();
        wx.request({
          url:app.globalData.config.apiUrl+ 'index.php?act=getRandomTeam',
          method:"POST",
          data:{
            openid:wx.getStorageSync("openid"),
            aid:aid
          ...
          success:res=>{
            ...
            if(res.data){
              setTimeout(function(){
                wx.redirectTo({
                  url: './shakeresult?myteam='+JSON.stringify(res.data.myteam),
                })
              },2000)
  ...
  to(){
    ...
    wx.request({
      ...
      success: res => {
        // console.log(res.data)
        console.log(JSON.stringify(res.data.myteam))
        if (res.data) {
          setTimeout(function () {
            wx.redirectTo({
              url: './shakeresult?aid='+aid+'&myteam=' + JSON.stringify(res.data.myteam),
            })
          }, 2000)
